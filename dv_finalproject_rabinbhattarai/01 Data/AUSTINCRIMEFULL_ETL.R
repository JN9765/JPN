...
file_path <- "AUSTINCRIMEFULL.csv"

atxCrimeS <- read.csv(file_path,stringsAsFactors = FALSE)
names(atxCrimeS)
str(atxCrimeS)
View(atxCrimeS)
df <- rename(atxCrimeS, Crime_Type = Crime.Type, Incident_report_number = Incident.Report.Number)
str(df)
...
if( length(measures) > 1 || ! is.na(dimensions)) {
  for(d in dimensions) {
    ...
    if(d == "Date") df[d] <- as.Date(atxCrimeS$Date,"%m/%d/%Y")
    else
